...
public class AuthServiceImpl implements IAuthService {
    ...
    public Token createToken(LoginRequest loginRequest) {
        ...
        if (loginTokens.size() >= SecurityConst.MAX_CLIENT_COUNT) {
            redisTemplate.delete(RedisUtils.formatKey(user.getAccount()));
        ...
        // 记录用户详情
        JwtUserDetail jwtUserDetail = new JwtUserDetail(user);
        RedisUtils.setStr(redisTemplate, AppConstants.CURRENT_USERINFO_DTO + user.getAccount(), jwtUserDetail.toJsonString(), token.getExpires(), TimeUnit.SECONDS);
        return token;
    }

    public void removeToken() {
        RedisUtils.setUserToken(redisTemplate, JwtTokenUtils.getCurrentUser(), AppConstants.JWT_TOKEN_PREFIX + JwtTokenUtils.getJWTToken(), false);
    }
